...
const SERVICE_ROUTE_REGISTRY: readonly ServiceRouteEntry[] = [
  ...
  {
    slug: 'balkony-i-terrasy',
    section: 'gidroizolyatsiya',
    title: 'Гидроизоляция балконов и террас в Крыму',
    excerpt:
      'Защищаем балконы и террасы от влаги: оцениваем основание, примыкания и покрытие, подбираем решение без лишних работ.',
    priceLabel: 'от 15 000 ₽ за объект',
    seo: {
      title: 'Гидроизоляция балконов и террас в Крыму — от 15 000 ₽',
      description:
        'Гидроизоляция балконов и террас в Симферополе и по Крыму от 15 000 ₽ за объект. Работаем с примыканиями, покрытием и проблемными зонами.',
      h1: 'Гидроизоляция балконов и террас в Крыму',
    },
  },
  {
    slug: 'lokalnyy-remont-i-germetizatsiya',
    section: 'gidroizolyatsiya',
    title: 'Локальный ремонт и герметизация узлов в Крыму',
    excerpt:
      'Закрываем локальные проблемы в узлах, примыканиях, швах и вводах, когда важно устранить слабое место без лишнего объёма работ.',
    priceLabel: 'от 7 000 ₽',
    seo: {
      title: 'Локальный ремонт и герметизация узлов в Крыму — от 7 000 ₽',
      description:
        'Локальный ремонт и герметизация узлов в Симферополе и по Крыму от 7 000 ₽. Работаем с примыканиями, швами, вводами и проблемными зонами.',
      h1: 'Локальный ремонт и герметизация узлов в Крыму',
    },
  },
  {
  ...
  },
  {
    ...
    excerpt:
      'Дополнительная услуга с понятным расчётом, аккуратным монтажом и простой подачей без лишней декоративности.',
    priceLabel: 'от 350 ₽/м²',
    seo: {
...
function toServicePageData(entry: ServiceRouteEntry): ServicePageData {
  return {
    ...
    whenUseful: [
      'Когда уже есть протечки, локальные повреждения или износ покрытия.',
      'Когда важно понять, можно ли обойтись ремонтом или восстановлением без полной замены.',
      'Когда задача связана с проблемными узлами, примыканиями и уязвимыми зонами.',
      'Когда нужен понятный следующий шаг по объекту: фотооценка, выезд, расчёт.',
    ],
    includes: [
      'Оценка состояния объекта и проблемной зоны.',
      'Подбор решения по фактическому состоянию покрытия.',
      'Работа с узлами, примыканиями и локальными повреждениями.',
      'Пояснение по составу и целесообразности работ.',
      'Ориентир по стоимости и следующему шагу.',
    ],
    advantages: [
      'Не навязываем лишний объём работ.',
      'Подбираем решение по состоянию объекта, а не по шаблону.',
      'Объясняем, когда хватает локального решения, а когда нужен больший объём.',
      'Работаем по Симферополю и Крыму.',
      'Держим фокус на кровле, восстановлении и гидроизоляции как на основном профиле.',
    ],
    faq: [
      {
        question: 'Можно ли получить предварительную оценку по фото?',
        answer:
          'Да. Пришлите 2–4 фото объекта, город и краткое описание задачи. После этого можно предварительно понять, какой формат работ целесообразен.',
      },
      {
        question: 'Вы сразу предлагаете замену?',
        answer:
          'Нет. Сначала оцениваем состояние объекта и только потом предлагаем ремонт, восстановление, гидроизоляцию или замену, когда она действительно нужна.',
      },
      {
        question: 'Работаете только по Симферополю?',
        answer:
          'Нет. Работаем по Симферополю и по Крыму.',
      },
    ],
  };
}
